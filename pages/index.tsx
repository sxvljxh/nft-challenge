...
const Home = ({ collections }: Props) => {
  return (
    <div className="max-w-7xl mx-auto flex slex-col min-h-screen py-20 px-10 2xl:px-0">
      <Head>
        <h1 className='mb-10 text-4xl font-extralight'>
          The <span className='font-extrabold underline decoration-pink-600/50'> Hasan's</span> NFT Marketplace.
        </h1>
        <main className='bg-slate-100 p-10 shadow-xl shadow-rows-400'>
              ...
              <Link href={`/nft/${collection.slug.current}`}>
                <div className='flex flex-col items-center transition-all duration-200 hover:scale-105'>
                  ...
                  <div className='p-5'>
                    ...
                    <p className='mt-2 text-gray-400'>{collection.description}</p>
                  </div>
                </div>
              </Link>
            ))}
